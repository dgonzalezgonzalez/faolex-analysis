...
log using "`out_dir'/logs/yellow_vests_policycount_methods.log", text replace
...
postfile `rs' str10 frequency str16 donor_spec str6 method double att se ci95_lb ci95_ub ///
    ...
    using "`out_dir'/yellow_vests_policycount_results_raw.dta", replace
...
foreach freq in annual quarterly {
    ...
    if "`freq'" == "annual" {
        ...
        collapse (count) policy_count = strategy_sus, by(country time)
        gen byte raw_obs = 1
    ...
    foreach donor in oecd oecd_exclshock {
        ...
        export delimited country time policy_count raw_obs tr_base if core_window == 1 using "`temp_dir'/yellow_vests_policycount_`freq'_panel_`donor'.csv", replace

        sort country_id time
        forvalues k = 1/`requested_lags' {
            by country_id (time): gen lag`k'_count = policy_count[_n-`k']
        }
        ...
        forvalues L = `requested_lags'(-1)1 {
            local ok = 1
            forvalues k = 1/`L' {
                quietly count if core_window == 1 & missing(lag`k'_count)
                if r(N) > 0 local ok = 0
            }
            if `ok' == 1 {
                local chosen_lags `L'
                local covars ""
                forvalues k = 1/`L' {
                    local covars `covars' lag`k'_count
                }
                continue, break
            }
        }
        ...
        foreach m in sdid sc {
            ...
            local gfile "`out_dir'/yellow_vests_policycount_`freq'_`m'_`donor'_trends.png"

            capture noisily {
                if "`m'" == "sdid" {
                    sdid policy_count country_id time tr if core_window == 1, vce(placebo) reps(`reps') seed(`seed') ///
                        covariates(`this_cov', projected) graph ///
                        g2_opt(xtitle("`freq' time") ytitle("Policy count") ///
                        title("France vs synthetic controls: policy count `=upper("`freq'")' `=upper("`m'")' `donor'"))
                }
                else {
                    sdid policy_count country_id time tr if core_window == 1, vce(placebo) reps(`reps') seed(`seed') ///
                        method(sc) graph ///
                        g2_opt(xtitle("`freq' time") ytitle("Policy count") ///
                        title("France vs synthetic controls: policy count `=upper("`freq'")' SC `donor'"))
                }
...
use "`out_dir'/yellow_vests_policycount_results_raw.dta", clear
order frequency donor_spec method att se ci95_lb ci95_ub chosen_lags n_countries n_controls n_obs rc covariates
export delimited using "`out_dir'/yellow_vests_policycount_results.csv", replace
erase "`out_dir'/yellow_vests_policycount_results_raw.dta"
