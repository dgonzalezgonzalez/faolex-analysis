...
log using "`log_dir'/membership_twfe_outcomes.log", text replace
    ...
    foreach org in oecd eu {
        ...
        * Event-study TWFE, baseline k=-1
        ...
        forvalues k = 0/`post_window' {
            ...
            post `es' ("`outcome'") ("`org'") (`k') (bb) (ss) (ll) (uu) ("post")
        }
...
* ------------------------------------------------------------
* Export event-study + figures
* ------------------------------------------------------------
use "`out_dir'/membership_twfe_outcomes_eventstudy_raw.dta", clear
sort outcome organization rel_time
export delimited using "`out_dir'/membership_twfe_outcomes_eventstudy_results.csv", replace
save "`out_dir'/membership_twfe_outcomes_eventstudy_results.dta", replace

levelsof outcome, local(outcomes)
foreach oo of local outcomes {
    levelsof organization if outcome == "`oo'", local(orgs)
    foreach org of local orgs {
        preserve
            keep if outcome == "`oo'" & organization == "`org'"
            sort rel_time
            twoway ///
                ...
                , yline(0, lcolor(maroon) lpattern(dash)) ///
                  ...
                  legend(off)
            graph export "`out_dir'/membership_`org'_twfe_`oo'_eventstudy.png", replace width(2200)
        restore
    }
}
