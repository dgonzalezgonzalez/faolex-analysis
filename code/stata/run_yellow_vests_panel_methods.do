...
log using "`out_dir'/logs/yellow_vests_panel_methods.log", text replace
...
* ------------------------------------------------------------
tempname rs
postfile `rs' str10 frequency str16 donor_spec str6 method double att se ci95_lb ci95_ub ///
    int chosen_lags int n_countries int n_controls int n_obs int rc str120 covariates ///
    using "`out_dir'/yellow_vests_methods_results_raw.dta", replace
...
foreach freq in annual quarterly {
    ...
    if "`freq'" == "annual" {
        ...
        collapse (mean) strategy_sus (count) policy_count = strategy_sus, by(country time)
    ...
    if "`freq'" == "quarterly" {
        ...
        * Carry-forward then backfill for empty quarters inside window
        bysort country_id (time): replace strategy_sus = strategy_sus[_n-1] if missing(strategy_sus)
        gsort country_id -time
        by country_id: replace strategy_sus = strategy_sus[_n-1] if missing(strategy_sus)
    ...
    * donor-spec loop
    foreach donor in oecd oecd_exclshock {

        preserve

        if "`donor'" == "oecd_exclshock" {
        ...
        * keep only countries with full support after donor filtering
        ...
        * Export panel snapshot for this frequency/donor set
        export delimited country time strategy_sus policy_count raw_obs tr_base ///
            if core_window == 1 using "`temp_dir'/yellow_vests_country_`freq'_panel_`donor'.csv", replace

        * Lags for SDiD covariates
        sort country_id time
        forvalues k = 1/`requested_lags' {
            by country_id (time): gen lag`k'_sus = strategy_sus[_n-`k']
        }
        ...
        * run method loop
        foreach m in sdid sc {
            ...
            local gfile "`out_dir'/yellow_vests_`freq'_`m'_`donor'_trends.png"

            capture noisily {
                if "`m'" == "sdid" {
                    sdid strategy_sus country_id time tr if core_window == 1, vce(placebo) reps(`reps') seed(`seed') ///
                        covariates(`this_cov', projected) graph ///
                        g2_opt(xtitle("`freq' time") ytitle("Mean strategy_sus") ///
                        title("France vs synthetic controls: `=upper("`freq'")' `=upper("`m'")' `donor'"))
                }
                else {
                    sdid strategy_sus country_id time tr if core_window == 1, vce(placebo) reps(`reps') seed(`seed') ///
                        method(sc) graph ///
                        g2_opt(xtitle("`freq' time") ytitle("Mean strategy_sus") ///
                        title("France vs synthetic controls: `=upper("`freq'")' SC `donor'"))
                }
...
use "`out_dir'/yellow_vests_methods_results_raw.dta", clear
order frequency donor_spec method att se ci95_lb ci95_ub chosen_lags n_countries n_controls n_obs rc covariates
export delimited using "`out_dir'/yellow_vests_methods_results.csv", replace
erase "`out_dir'/yellow_vests_methods_results_raw.dta"
