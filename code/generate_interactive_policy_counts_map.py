...
def generate_interactive_map(input_path: Path, output_path: Path):
    ...
    # Drop missing countries and ensure string type
    df = df.dropna(subset=['country'])
    df['country'] = df['country'].astype(str)

    # Build a consistent country -> ISO3 mapping
    # For each unique country, get ISO3 code (handling multi-matches by taking first)
    def safe_countrycode(names):
        """Map country names to ISO3, returning first match for multi-matches."""
        results = countrycode.countrycode(names, 'country.name', 'iso3c')
        cleaned = []
        for r in results:
            if isinstance(r, (list, tuple)):
                cleaned.append(r[0] if len(r) > 0 else None)
            else:
                cleaned.append(r)
        return cleaned

    unique_countries = df[['country']].drop_duplicates()
    unique_countries['iso3'] = safe_countrycode(unique_countries['country'].tolist())
    # Keep only countries that successfully mapped
    unique_countries = unique_countries[unique_countries['iso3'].notna()]
    # Join back to get consistent ISO3 for each country
    df = df.merge(unique_countries[['country', 'iso3']], on='country', how='inner')

    logger.info(f"Using {len(df)} records, years {df.year.min()}-{df.year.max()}, {df.country.nunique()} countries (with unique ISO3 mapping)")

    # Aggregate counts by country-year (group by country name + iso3, where iso3 is now consistent per country)
    ...
    counts = (
        ...
        .apply(lambda x: x.assign(count=x['count'].cumsum()))
        .reset_index()  # This keeps 'country' and 'iso3' as columns
    )
